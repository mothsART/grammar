...
correspondant_test = {
    "start_of_paragraph"        : "un texte   avec   trop d'espaces",
    "end_of_paragraph"          : "un texte avec trop d'espaces à la fin    ",
    "between_words"             : "un texte   avec    trop  d'espaces",
    "before_punctuation"        : "un texte avec trop d'espaces dans la ponctuation  .  ;  !  ;  …",
    "within_parenthesis"        : "(  un texte avec trop d'espaces à l'intérieur des parenthèses  )",
    "within_square_brackets"    : "[  un texte avec trop d'espaces à l'intérieur des crochets   ]",
    "within_quotation_marks"    : "“ un texte avec trop d'espaces à l'intérieur des guillements  ”"
}

def loop_formater():
    for key in dReplTable.keys():
        for rule in dReplTable[key]:
            re.sub(rule[0], rule[1], correspondant_test[key])
